...
import {
  LayoutDashboard, Package, Layers, BarChart3, Users, Heart,
  ShoppingCart, Smartphone, UserCog, MessageSquare, Settings,
  LogOut, PanelLeft, Store, ClipboardList,
} from "lucide-react";
...
const menuGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: ClipboardList, label: "Till Sessions", path: "/till" },
    ],
  },
  {
    label: "Customers",
...
function DashboardLayoutContent({
  ...
  const activeMenuItem = allMenuItems.find(item => location === item.path || (item.path !== "/" && location.startsWith(item.path)));
              ...
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-3 rounded-lg px-1 py-1 hover:bg-accent/50 transition-colors w-full text-left group-data-[collapsible=icon]:justify-center focus:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                  ...
                  <div className="flex-1 min-w-0 group-data-[collapsible=icon]:hidden">
                    ...
                    <p className="text-[11px] text-muted-foreground truncate mt-1">{user?.role ?? "staff"}</p>
                  </div>
                </button>
              </DropdownMenuTrigger>
