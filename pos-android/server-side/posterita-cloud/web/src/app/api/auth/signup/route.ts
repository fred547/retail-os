...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1b. Detect country from IP → look up country_config for region + currency
    const ipCountryCode = req.headers.get("x-vercel-ip-country") || "";
    let detectedCountryCode: string | null = null;
    let detectedBillingRegion = "developed"; // Safe default
    let detectedCurrency = currency; // Use user-supplied currency as fallback

    if (ipCountryCode) {
      const { data: countryConfig } = await supabase
        .from("country_config")
        .select("country_code, billing_region, currency_code")
        .eq("country_code", ipCountryCode.toUpperCase().trim())
        .maybeSingle();

      if (countryConfig) {
        detectedCountryCode = countryConfig.country_code;
        detectedBillingRegion = countryConfig.billing_region;
        // Use detected currency if user didn't explicitly provide one
        if (!body.currency) {
          detectedCurrency = countryConfig.currency_code;
        }
      }
    }

    // If no IP detection, try matching by the user-supplied country name
    if (!detectedCountryCode && country) {
      ...
      if (countryByName) {
        detectedCountryCode = countryByName.country_code;
        detectedBillingRegion = countryByName.billing_region;
        if (!body.currency) {
          detectedCurrency = countryByName.currency_code;
        }
      }
    }
    ...
    const { error: liveAccErr } = await supabase.from("account").insert({
      ...
      billing_region: detectedBillingRegion,
    });
    if (liveAccErr) {
      return NextResponse.json({ error: "Operation failed" }, { status: 500 });
    }
    ...
    // 9. Grant 14-day Growth trial to the live account
    try {
      ...
      await supabase.from("account").update({
        ...
        trial_granted_by: "system",
      }).eq("account_id", liveAccountId);
    } catch (_) { /* non-blocking — account still works on free plan */ }

    // 10. Read back sync_secret for the live account (auto-generated by DB default)
