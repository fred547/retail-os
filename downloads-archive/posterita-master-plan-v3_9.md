# Posterita Unified Platform — Master Plan v4.0
...
| **v3.9** | **19 Mar 2026** | **Open-to-Buy (OTB) planning with selling periods, budget envelopes per category/period, stock cover (months of supply) dashboards per store and warehouse, PO period tagging, arrival timeline Gantt, revised OTB in-season recalculation. 8 roles, 42 sections.** |
| **v4.0** | **20 Mar 2026** | **Web console navigation structure decided: Products & Customers are top-level apps; all admin/config grouped under Settings. Android POS is read-only consumer of configs — no admin management on device.** |
...
| Web Console | Next.js on Vercel | Product & customer management (top-level apps), admin config (Settings: stores, terminals, staff, roles, tax, payments, receipts, integrations), approvals, reconciliation, reports, campaigns, operations oversight |
...
| Create/edit selling periods | ✓ | ✓ | | ✓ | | | | |
| Set OTB budgets | ✓ | ✓ | | ✓ | | | | |
| View OTB burn-down | ✓ | ✓ | ✓ | ✓ | ✓ | | | |
| View stock cover dashboard | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | | |
| View arrival timeline | ✓ | ✓ | ✓ | ✓ | | | | |
| Override OTB warning on PO | ✓ | ✓ | | | | | | |
| Lock/close selling period | ✓ | ✓ | | | | | | |
...
| `otb-planning` | **New:** selling periods, OTB budget CRUD, stock cover materialized views, PO period tagging validation, revised OTB weekly recalculation, arrival timeline queries | New module |
...
| Stock Cover | Merchandiser, Managers, Supervisors | Stock months heatmap per store × category, warehouse forward cover, category-level stock cover targets |
...
### ✅ RESOLVED: Web Console Navigation Structure (v4.0)

The 26+ sections are organized into a clear super-app navigation. **Products** and **Customers** are top-level apps (used daily). All admin/config items are grouped under **Settings** (used rarely, during setup).

**Top-level navigation:**

| Nav Item | What it contains |
|---|---|
| **Dashboard** | KPIs, alerts, live activity |
| **Orders** | Transaction history, refunds, order lookup |
| **Products** | Full catalog management — items, categories, pricing, images, barcodes, catalogue PDF |
| **Customers** | CRM, loyalty wallets, consent, audience, campaigns |
| **Reports** | Sales, inventory, reconciliation, staff, device health |
| **Settings** | Store & Terminals, Staff & Roles, Tax & Pricing, Payments, Receipts, Integrations (Blink, WhatsApp, Cloudinary) |

**Phase-specific sub-sections within Settings:**

- **MVP:** Store & Terminals, Staff & Roles, Tax & Pricing, Payments, Receipts
- **Phase 3+:** Procurement & Vendors, AI Setup, Data Compliance, Exchange Rates
- **Phase 4+:** Merchandise Planning (OTB, Stock Cover, Arrival Timeline as sub-tabs)

**Android POS is a read-only consumer** of all configs managed in the web console. It syncs products, customers, tax rules, and payment config via the backend API. No admin management screens on the Android device — only lightweight actions like quick-add customer at checkout.

**Note:** OTB Planning, Stock Cover, and Arrival Timeline are sub-tabs within a single "Merchandise Planning" section under Settings to reduce navigation clutter.
...
| 43 | Web Console Navigation (v4.0) | Super-app nav: Dashboard, Orders, Products, Customers, Reports as top-level apps. Settings groups all admin config (Store & Terminals, Staff & Roles, Tax & Pricing, Payments, Receipts, Integrations). Android POS is read-only consumer of configs — no admin screens on device. |
...
CREATE TABLE purchase_order (
    ...
    selling_period_id UUID REFERENCES selling_period(id),    -- OTB: which period this PO buys for
...
CREATE TABLE purchase_order_line (
    ...
    selling_period_id UUID REFERENCES selling_period(id),    -- OTB: line-level period override (a single PO can span periods)
...
"Rs 340K left      sandals for    3 vendors.        becomes PO.         inspection,
 for footwear/     summer"        Purchaser sends   Manager approves.   costing,
 August"           (tagged to     RFQ emails.       PO tagged to        release.
                    period)       Vendors reply      selling period.     OTB + stock
                                  to our inbox.      OTB burns down.    cover update.
```
...
CREATE TABLE sourcing_requirement (
    ...
    selling_period_id   UUID REFERENCES selling_period(id), -- OTB: which period this sourcing targets
...
## 42. Open-to-Buy (OTB) & Stock Cover Planning

### The Problem

The procurement pipeline (§40) handles the mechanics of buying: sourcing → RFQ → PO → receive. But there is no **financial guardrail** telling the merchandiser how much they are allowed to buy for a given period, and no **visibility** into whether current stock levels are healthy, critical, or bloated. Purchases happen reactively rather than against a plan. The merchandiser needs to know: "How much can I still buy for August?", "How many months of sandal stock do we have at Port Louis store?", and "When will my orders actually arrive?"
...
```
PLAN                    BUY                     RECEIVE              SELL
─────────────────────   ──────────────────────   ──────────────────   ──────────
Selling Period          Sourcing Requirement     Container (§35)      POS (§18)
  ↓                       ↓                       ↓                    ↓
OTB Budget              RFQ → PO                 Goods In             Sales Data
  (per category/period)   (tagged to period)       (updates stock)      (feeds back
  ↓                       ↓                       ↓                    into next
Stock Cover Target      OTB consumed             Stock Cover          period's
  (3-6 months)            (budget burns down)      recalculates         forecast)
```

### 42.1 Selling Periods

A selling period is a brand-configurable time window. Most retailers use monthly or seasonal buckets. The brand defines their own calendar.

```sql
CREATE TABLE selling_period (
    id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    organization_id     UUID NOT NULL REFERENCES organization(id),
    name                TEXT NOT NULL,                -- "Jul 2026", "Summer 2026", "Q3 2026"
    period_type         TEXT NOT NULL DEFAULT 'month'
                        CHECK (period_type IN ('month', 'quarter', 'season', 'custom')),
    starts_at           DATE NOT NULL,
    ends_at             DATE NOT NULL,
    status              TEXT NOT NULL DEFAULT 'planning'
                        CHECK (status IN ('planning', 'open', 'in_season', 'closed')),
    -- Once "in_season", actuals start flowing in and plan becomes blended
    created_by          UUID NOT NULL REFERENCES "user"(id),
    created_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
...
- `in_season` — period has started, actuals flowing in, revised OTB recalculates weekly
- `closed` — period ended, performance locked for reporting

Brands can auto-generate periods (e.g., "Create monthly periods for Jul–Dec 2026") or define custom seasons ("Festive 2026: Nov 15 – Jan 15").

### 42.2 OTB Budget Plan

The financial envelope. The merchandiser sets targets per **category** per **selling period**.
...
```sql
CREATE TABLE otb_plan (
    id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    organization_id     UUID NOT NULL REFERENCES organization(id),
    selling_period_id   UUID NOT NULL REFERENCES selling_period(id),
    category            TEXT NOT NULL,                -- product category or "ALL"
    -- Plan targets (set by merchandiser, in local currency at cost)
    planned_sales       NUMERIC(14,2) NOT NULL DEFAULT 0,   -- expected sell-through at cost
    planned_eom_stock   NUMERIC(14,2) NOT NULL DEFAULT 0,   -- desired end-of-period inventory at cost
    planned_markdowns   NUMERIC(14,2) NOT NULL DEFAULT 0,   -- expected markdowns/shrinkage at cost
    -- Calculated fields (system-maintained)
    bom_stock           NUMERIC(14,2) NOT NULL DEFAULT 0,   -- beginning-of-period stock at cost (snapshotted)
    on_order            NUMERIC(14,2) NOT NULL DEFAULT 0,   -- committed POs for this period + category
    otb_amount          NUMERIC(14,2) GENERATED ALWAYS AS (
                            (planned_sales + planned_eom_stock + planned_markdowns)
                            - (bom_stock + on_order)
                        ) STORED,
    -- Reserve: merchandiser holds back a % for opportunistic buys
    reserve_pct         NUMERIC(4,2) NOT NULL DEFAULT 15.0, -- default 15%
    otb_available       NUMERIC(14,2) GENERATED ALWAYS AS (
                            ((planned_sales + planned_eom_stock + planned_markdowns)
                             - (bom_stock + on_order))
                            * (1 - reserve_pct / 100)
                        ) STORED,
    ...
    actual_markdowns    NUMERIC(14,2) NOT NULL DEFAULT 0,
    actual_receipts     NUMERIC(14,2) NOT NULL DEFAULT 0,   -- goods actually received at cost
    -- Revised OTB (live, recalculated weekly during in_season)
    revised_otb         NUMERIC(14,2),                      -- adjusted based on actuals vs plan
    revised_at          TIMESTAMPTZ,                        -- last recalculation timestamp
    -- Target stock cover
    target_stock_months NUMERIC(4,1) NOT NULL DEFAULT 4.0,  -- brand-configurable: 3-6 months
    -- Performance (set at period close)
    eom_stock_actual    NUMERIC(14,2),                      -- actual end-of-period stock
    variance_pct        NUMERIC(5,2),                       -- (actual EOM - planned EOM) / planned EOM × 100
    performance_grade   TEXT CHECK (performance_grade IN ('excellent', 'good', 'fair', 'poor')),
    -- ±5% = excellent, ±10% = good, ±20% = fair, >20% = poor
    -- Metadata
    locked              BOOLEAN NOT NULL DEFAULT false,      -- lock when finalized
    notes               TEXT,
    created_by          UUID NOT NULL REFERENCES "user"(id),
    created_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
    UNIQUE (organization_id, selling_period_id, category)
);
```

**Key behaviors:**

- `bom_stock` is auto-snapshotted from inventory when the period transitions to `open` or `in_season`
- `on_order` is a live aggregate of all approved POs tagged to this period + category (sum of `purchase_order_line.line_total` where `selling_period_id` matches)
- `otb_amount` is a generated column — always current
- `otb_available` subtracts the reserve — this is what the merchandiser actually sees as "available to buy"
- `revised_otb` is recalculated weekly once the period is `in_season`, blending actuals with remaining plan
- `target_stock_months` defaults to 4.0 but is configurable per category (fashion = 2-3 months, staples = 5-6 months)
- `performance_grade` is computed at period close using the ±5% industry standard

### 42.3 PO Period Tagging & OTB Validation

Every Purchase Order must declare which selling period it buys for (added to `purchase_order.selling_period_id` in §35/§40 schemas). Line-level overrides via `purchase_order_line.selling_period_id` allow a single PO to span periods (e.g., 200 sandals for "Summer 2026", 100 boots for "Winter 2026").

**Validation rule on PO approval:**

```
WHEN PO transitions to 'approved':
  FOR EACH line in PO:
    period = line.selling_period_id ?? po.selling_period_id
    category = product.category for this line
    otb_plan = lookup(organization_id, period, category)

    IF (otb_plan.on_order + line.line_total) > otb_plan.otb_available:
      → FLAG: "This PO exceeds OTB budget for {category} / {period.name}"
      → Show: "Budget: {otb_available}, Already committed: {on_order}, This PO adds: {line_total}"
      → Soft block: Purchaser sees warning, cannot approve
      → Owner/Admin can override with reason (logged to audit_event)
```

This is a **soft guard, not a hard block**. The business may intentionally overspend on a category — but the system makes it visible and auditable.

### 42.4 Stock Cover (Months of Supply)

The "how many months of stock do we have?" metric. Calculated per product category, per location (each store individually, plus warehouse).
...
Where N = `organization.stock_cover_lookback_months` (default 3, configurable).

```sql
-- Organization-level stock cover configuration
ALTER TABLE organization
    ADD COLUMN default_stock_cover_target NUMERIC(4,1) DEFAULT 4.0,
    ADD COLUMN stock_cover_lookback_months INTEGER DEFAULT 3
        CHECK (stock_cover_lookback_months BETWEEN 1 AND 12);

-- Materialized view, refreshed daily via pg_cron (or on-demand trigger)
CREATE MATERIALIZED VIEW mv_stock_cover AS
WITH monthly_sales AS (
    SELECT
        sa.store_id,
        p.category,
        SUM(sl.quantity * COALESCE(p.manual_cost_price, p.system_cost_price, 0))
            / GREATEST(
                EXTRACT(EPOCH FROM (now() - (now() - (o.stock_cover_lookback_months || ' months')::INTERVAL)))
                / (30.44 * 86400),  -- convert to months
                1
            ) AS avg_monthly_cost
    FROM sale_line sl
    ...
    WHERE sa.created_at >= now() - (o.stock_cover_lookback_months || ' months')::INTERVAL
      AND sa.status = 'completed'
    GROUP BY sa.store_id, p.category, o.stock_cover_lookback_months
),
current_stock AS (
    SELECT
        si.store_id,
        p.category,
        SUM(si.quantity * COALESCE(p.manual_cost_price, p.system_cost_price, 0)) AS stock_value
    FROM stock_item si
    JOIN product p ON p.id = si.product_id
    WHERE p.product_class = 'resale'
      AND si.quantity > 0
    GROUP BY si.store_id, p.category
)
SELECT
    cs.store_id,
    ...
    ms.avg_monthly_cost,
    CASE
        WHEN COALESCE(ms.avg_monthly_cost, 0) = 0 THEN NULL  -- no sales history
        ELSE ROUND(cs.stock_value / ms.avg_monthly_cost, 1)
    END AS stock_months,
    o.default_stock_cover_target AS target_months
FROM current_stock cs
JOIN store s ON s.id = cs.store_id
JOIN organization o ON o.id = s.organization_id
LEFT JOIN monthly_sales ms ON ms.store_id = cs.store_id AND ms.category = cs.category;

-- Index for fast dashboard queries
CREATE UNIQUE INDEX ON mv_stock_cover (store_id, category);
```

**Warehouse-level stock cover** uses the same formula but aggregates all stores' sell-through rates as the denominator. If the warehouse holds 10,000 units of sandals and all stores combined sell 2,500/month, the warehouse has 4 months of forward cover.

**Refresh schedule:** Materialized view refreshed nightly by `pg_cron`. Manual refresh button available in the dashboard for on-demand recalculation.

### 42.5 Stock Cover Dashboard — Heatmap

A matrix grid on the web console:

```
                    Footwear    Apparel    Accessories    Electronics    ...
─────────────────── ─────────── ────────── ────────────── ────────────── ────
Port Louis Store    🟢 4.2 mo   🔴 1.3 mo  🟡 2.8 mo      🟢 3.9 mo
Curepipe Store      🔵 6.1 mo   🟢 3.5 mo  🔴 0.9 mo      🟡 2.5 mo
Flic en Flac        🟡 2.6 mo   🟢 4.0 mo  🟢 3.7 mo      🔵 5.8 mo
─── WAREHOUSE ───   🟢 8.2 mo   🟡 5.1 mo  🟢 7.0 mo      🟢 9.3 mo
```
...
|---|---|---|
| 🔴 Red | < 50% of target (< 2.0 mo) | Critical — will stockout soon |
| 🟡 Amber | 50-80% of target (2.0-3.2 mo) | Low — order soon |
| 🟢 Green | 80-120% of target (3.2-4.8 mo) | Healthy |
| 🔵 Blue | > 120% of target (> 4.8 mo) | Overstocked — slow down buying |

Clicking any cell drills into the product breakdown: which specific SKUs are driving the number.

### 42.6 OTB Burn-Down Dashboard

A waterfall chart per period showing:

```
FOOTWEAR — AUGUST 2026

Total OTB Budget          ████████████████████████████████  Rs 400,000
─ Reserved (15%)          ░░░░░                             Rs  60,000
= Available               ████████████████████████████      Rs 340,000
─ Committed (3 POs)       ████████████████                  Rs 220,000
─ Received (1 container)  ██████████                        Rs 140,000
= Remaining to buy        ████████                          Rs 120,000  [GREEN]
```

**Status indicators:**
- Green: > 20% remaining — healthy
- Amber: 5-20% remaining — almost fully committed
- Red: 0% or over-committed — no more budget (or over-bought)

### 42.7 Arrival Timeline (Gantt)

A horizontal timeline showing all open POs:
...
JUN ──────── JUL ──────── AUG ──────── SEP ──────── OCT
    │                                                    │
    ├─ PO-042 [Summer] ▓▓▓▓▓▓▓▓▓▓████                  │  vendor confirmed → in transit → ETA Jul 20
    │                                                    │
    ├─ PO-048 [Summer] ▓▓▓▓▓▓░░░░░░░░░░████             │  RFQ sent → awaiting vendor
    │                                                    │
    ├─ PO-051 [Winter]              ▓▓▓▓▓▓▓▓▓▓▓████     │  vendor confirmed → ETA Sep 5
    │                                                    │
    ▼ MUST-ORDER-BY: Summer items = Jun 15 [PASSED]      │
      Winter items = Aug 1 [21 days remaining]           │
```

**Features:**
- Each PO rendered as a horizontal bar from `created_at` to `expected_delivery`
- Color = selling period (e.g., Summer = orange, Winter = blue)
- Bar segments show status: draft (░), sent to vendor (▒), confirmed (▓), shipped (█)
- "Today" vertical marker
- **Must-order-by** markers: calculated as `period.starts_at − vendor.average_lead_time`. If the date has passed and OTB budget remains uncommitted, the marker turns red — the merchandiser has missed the buying window for that period.
- Clicking a PO bar opens the PO detail screen
- ETA slip detection: if `expected_delivery` is updated to a later date, an amber triangle appears on the bar

### 42.8 Revised OTB (In-Season Recalculation)

Once a period is `in_season`, the system runs a weekly recalculation every Monday:
...
Where:
  Revised Sales Forecast = actual_sales_so_far + (planned_remaining_weeks × adjusted_weekly_rate)
  Adjusted weekly rate = trending rate from last 4 weeks, not original plan
```

**Notification to merchandiser:**

```
📊 Weekly OTB Update — Footwear / August 2026
   Planned sales: Rs 400,000
   Actual to date: Rs 280,000 (70% with 2 weeks remaining)
   Trend: tracking 12% above plan
   → Revised OTB: Rs 45,000 (was Rs 120,000)
   → Recommendation: Consider reducing September intake
```

### 42.9 The Merchandiser End-to-End Workflow

```
1. PLAN (pre-season, 2-6 months out)
   ┌────────────────────────────────────────────────────────────┐
   │ Owner/Merchandiser creates selling periods for next 6 mo  │
   │ ↓                                                          │
   │ Sets OTB budget per category per period                    │
   │ (informed by last year actuals + growth % + stock target)  │
   │ ↓                                                          │
   │ System: "You need Rs 2.4M footwear Jul-Dec,                │
   │          distributed ~Rs 400K/month"                       │
   │ ↓                                                          │
   │ Merchandiser sets reserve at 15% per period                │
   │ ↓                                                          │
   │ Stock cover dashboard confirms: "Port Louis at 1.8 mo     │
   │ footwear — below target. Prioritize August buying."        │
   └────────────────────────────────────────────────────────────┘

2. BUY (against the plan)
   ┌────────────────────────────────────────────────────────────┐
   │ Merchandiser creates Sourcing Requirement                  │
   │ → Tags to selling period "August 2026"                     │
   │ → System: "OTB remaining for Footwear / Aug: Rs 340,000"  │
   │ ↓                                                          │
   │ Purchaser runs RFQ → accepts quote → PO created            │
   │ → PO inherits selling_period_id from sourcing requirement  │
   │ → OTB burn-down updates immediately                        │
   │ → If PO would exceed OTB: warning on approval screen       │
   │ → Owner can override with logged reason                    │
   └────────────────────────────────────────────────────────────┘

3. TRACK (in-transit)
   ┌────────────────────────────────────────────────────────────┐
   │ Arrival Timeline shows all open POs on Gantt               │
   │ → "PO-042 (500 sandals for Aug): in transit, ETA Jul 20"  │
   │ → If ETA slips past period start: amber alert              │
   │ → Must-order-by markers flag missed buying windows         │
   │ → Merchandiser can reassign PO to different period         │
   └────────────────────────────────────────────────────────────┘

4. RECEIVE (goods in)
   ┌────────────────────────────────────────────────────────────┐
   │ Container arrives → inspection → stock released (§35)      │
   │ → actual_receipts on OTB plan updates                      │
   │ → Stock cover recalculates across all stores               │
   │ → Dashboard: "Port Louis now at 3.8 mo footwear [green]"  │
   └────────────────────────────────────────────────────────────┘

5. SELL (in-season)
   ┌────────────────────────────────────────────────────────────┐
   │ POS transactions flow in daily                             │
   │ → actual_sales on OTB plan updates nightly                 │
   │ → Revised OTB recalculates every Monday                    │
   │ → "Tracking 12% above plan — reduce next month's buy"     │
   │ → Stock cover dashboard shows real-time months remaining   │
   └────────────────────────────────────────────────────────────┘

6. CLOSE (period end)
   ┌────────────────────────────────────────────────────────────┐
   │ Period transitions to 'closed'                             │
   │ → EOM stock snapshotted → becomes next period's BOM stock  │
   │ → Variance calculated: actual EOM vs planned EOM           │
   │ → Performance grade assigned (±5% = excellent)             │
   │ → Data feeds into next period's planning baseline          │
   └────────────────────────────────────────────────────────────┘
```

### 42.10 Best Practice Rules (Enforced by System)

| Rule | Implementation |
|---|---|
| **Never commit 100% of OTB upfront** | `reserve_pct` defaults to 15%. OTB burn-down shows "committed" vs "reserved" vs "available" as three distinct segments. System prevents tagging POs against the reserve unless Owner overrides. |
| **Stock cover targets vary by category** | `otb_plan.target_stock_months` is set per category/period. Fashion = 2-3 months, staples = 5-6 months. Heatmap uses per-category targets, not a global number. |
| **Weekly OTB review cadence** | `pg_cron` job every Monday at 06:00 UTC recalculates `revised_otb` for all `in_season` periods. Push notification to Merchandiser. |
| **The 5% rule** | At period close, `variance_pct` is computed. ±5% = "excellent", ±10% = "good", ±20% = "fair", >20% = "poor". Shown on period performance report. |
| **Lead time awareness** | Must-order-by = `period.starts_at − vendor.average_lead_time`. If date has passed and OTB uncommitted, arrival timeline shows red marker. |
| **OTB is iterative, not set-and-forget** | Status-based lifecycle ensures plans are revisited as actuals flow in. System won't let a period sit in "planning" past its `starts_at` date. |
...
GET    /v1/otb/plans/{id}/burn-down             — Burn-down chart data (budget, committed, received, remaining)
POST   /v1/otb/plans/recalculate                — Trigger revised OTB recalculation for in-season periods

-- Stock Cover
GET    /v1/otb/stock-cover                      — Heatmap data: stock months per store × category
...
### 42.12 Reports (added to §27)

- **OTB Performance by Period** — planned vs actual for each category/period, variance %, performance grade
- **Stock Cover Trend** — monthly stock cover history per store, showing whether stock health is improving or deteriorating
- **OTB Utilization** — % of budget committed, received, and remaining across all open periods
- **Arrival Forecast** — expected receipts by week for the next 3 months, overlaid on sales forecast
- **Must-Order-By Alert Summary** — upcoming and missed buying deadlines per period/category

### 42.13 Web Console Section

Add to §13 sections (implemented as sub-tabs within a single **Merchandise Planning** section):

| Sub-tab | Functions |
|---|---|
| **OTB Planning** | Selling period calendar, OTB budget entry per category/period, burn-down waterfall chart, reserve management, revised OTB tracker, period performance at close |
| **Stock Cover** | Heatmap matrix (stores × categories), warehouse forward cover, drill-down to SKU level, target configuration per category, trend sparklines |
| **Arrival Timeline** | Gantt chart of open POs, must-order-by markers, ETA slip detection, period-tagged color coding, click-through to PO detail |

### 42.14 Open Items

| # | Item | Target | Status |
|---|---|---|---|
| 1 | Define default selling period type per organization (month vs quarter vs season) | Pre-implementation | Decision needed |
| 2 | Determine whether stock cover should use cost price or retail price as basis | Pre-implementation | Recommend cost (aligns with OTB at cost), confirm with accountant |
| 3 | Define alert thresholds for must-order-by notifications (how many days before deadline?) | Pre-implementation | Suggest 14 days warning, 7 days critical |
| 4 | Decide whether OTB soft block on PO approval requires Owner or Admin+Owner override | Pre-implementation | Suggest Owner only for > 20% over-budget, Admin for < 20% |
| 5 | Determine `pg_cron` schedule for mv_stock_cover refresh and revised OTB recalculation | Phase 3 | Default: stock cover nightly at 02:00, revised OTB Mondays 06:00 |
| 6 | Define historical data seeding strategy — how to bootstrap OTB plans for brands with no sales history | Phase 3 | Suggest manual estimate + industry benchmarks for first 2 periods |

---

*This is a living document. Update as decisions are made and implementation reveals new constraints.*
*v3.9 — 19 March 2026 — 42 sections — OTB & Stock Cover Planning with selling periods, budget envelopes, stock months dashboards, arrival timeline*
